...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    adminAPI.users({ page, role: role || undefined, search: search || undefined })
      .then(({ data }) => {
        if (!active) return
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 animate-fade-in">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-slate-500">Browse customer, provider, driver, and admin accounts.</p>
      </div>
      ...
      <div className="card overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {loading ? Array.from({ length: 6 }).map((_, index) => (
            ...
            <div key={user._id} className="px-5 py-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                ...
                <span className={`badge ${user.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
